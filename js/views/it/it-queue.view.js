    ...
    if (filtered.length === 0) {
      ...
      container.innerHTML = `
        <div class="bg-white rounded-2xl border border-[#E9E3DD] p-10 sm:p-14 text-center flex flex-col items-center justify-center shadow-2xs space-y-3.5 my-4 flex-1 animate-empty-state">
          ...
          <h3 class="font-heading font-bold text-base sm:text-lg text-[#3E2B1E] tracking-tight">No IT Tickets Found</h3>
          <p class="text-xs sm:text-sm text-[#6F5849] max-w-md mx-auto leading-relaxed">
            There are no IT dispatch tickets matching your active search or filters.
          </p>
          <div class="flex items-center justify-center gap-2 pt-2">
            <button onclick="app.resetITQueueFilters()" class="btn-secondary h-9 px-4 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 border border-[#E9E3DD] bg-white hover:bg-[#FAF7F4] text-[#3E2B1E] transition cursor-pointer shadow-2xs active:scale-[0.98]">
