    ...
    if (filtered.length === 0) {
      ...
      container.innerHTML = `
        <div class="bg-white rounded-2xl border border-[#E9E3DD] p-10 sm:p-14 text-center flex flex-col items-center justify-center shadow-2xs space-y-3.5 my-4 flex-1 animate-empty-state">
          ...
          <h3 class="font-heading font-bold text-base sm:text-lg text-[#3E2B1E] tracking-tight">No Private Room Requests Found</h3>
          <p class="text-xs sm:text-sm text-[#6F5849] max-w-md mx-auto leading-relaxed">
            There are no private room approval requests matching your current filters.
          </p>
          <div class="flex items-center justify-center gap-2 pt-2">
            <button onclick="app.resetOwnerFilters()" class="btn-secondary h-9 px-4 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 border border-[#E9E3DD] bg-white hover:bg-[#FAF7F4] text-[#3E2B1E] transition cursor-pointer shadow-2xs active:scale-[0.98]">
              <span class="iconify text-xs text-[#7D6857]" data-icon="lucide:rotate-ccw" data-stroke-width="2"></span>
              <span>Reset Filters</span>
            </button>
            <button onclick="app.seedSampleOwnerRequests()" class="btn-primary h-9 px-4 rounded-xl text-xs font-bold text-white flex items-center justify-center gap-2 bg-[#991B1B] hover:bg-[#7F1D1D] transition shadow-xs cursor-pointer active:scale-[0.98]">
              <span class="iconify text-xs text-white" data-icon="lucide:database" data-stroke-width="1.8"></span>
